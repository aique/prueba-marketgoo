...
$app->map(["GET", "POST"], "/graphql", function(Request $request, Response $response) {
    ...
    $aggregatorSolver = new AggregatorSolver([new IpRegionAggregator($regionFinderFactory)]);
                ...
                "query" => new ObjectType([
                    ...
                    "fields" => [
                        "user" => [
                            ...
                            "resolve" => function ($rootValue, $args) use ($users, $aggregatorSolver) {
                                return isset($users[intval($args["id"])])
                                    ? $aggregatorSolver->getEntity($users[intval($args["id"])])
                                    : null;
                            }
                        ],
                        "users" => [
                            "type" => Type::listOf($graphql_user_type),
                            "resolve" => function() use ($users, $aggregatorSolver) {
                                return $aggregatorSolver->getCollection($users);
                            }
                        ]
                    ]
                ])
